...
function UserDetail() {
    ...
    const updateUser = (userDetail) => {
        return axios.put(`${BASE_URL}/api/users/${userId}`, userDetail, {
            headers: {
    ...
    useEffect(() => {
        const loadUserDetail = async () => {
            const result = await axios.get(`${BASE_URL}/api/users/${userId}`, {
                headers: {
    ...
    return (
        ...
        <div className='profile-container mt-3 user-detail'>

            <h1 className='user-detail-title'>User Detail</h1>
            <div className='d-flex'>
                {!edit ? <button className='ms-auto d-flex edit-btn' onClick={() => setEdit(true)}>Edit</button> : null}
            </div>
